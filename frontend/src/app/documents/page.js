...
function HomePage() {
  const [activeNavItem, setActiveNavItem] = useState("documents"); // State for active nav item
...
const closeModal = () => {
      ...
      if (response.ok) {
        ...
        if (jsonMatch) {
          ...
          setFormData({
            ...
            due_date_for_present_month:jsonData.due_date_for_present_month || "",
  ...
  useEffect(() => {
    ...
    setActiveNavItem(pathToNavItem[router.pathname] || "documents");
  }, [router.pathname]);
